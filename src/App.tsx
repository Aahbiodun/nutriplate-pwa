...
export default function App() {
  // --- State ---
  const [activeTab, setActiveTab] = useState('log'); // 'log', 'foods', 'plates'
  ...
  useEffect(() => {
    ...
    const fallbackTimer = setTimeout(() => {
      setIsLoading(false);
      console.warn("Firebase sync timeout - bypassing loading screen.");
    }, 3000);
  ...
  const HomeView = () => (
    <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-300">
      <div className="bg-emerald-600 text-white rounded-b-[2.5rem] p-6 pb-12 shadow-md relative z-10">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center space-x-2 font-medium">
            <Calendar size={18} />
            <span>
              {currentDate === getTodayString() ? 'Today' : new Date(currentDate).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })}
            </span>
          </div>
        ...
        <div className="text-center">
          <p className="text-emerald-100 text-sm font-medium uppercase tracking-wider mb-1">Total Consumed</p>
          <div className="flex justify-center items-baseline space-x-1">
            <span className="text-6xl font-bold tracking-tight">{Math.round(totalDailyCalories) || 0}</span>
            <span className="text-xl text-emerald-200">kcal</span>
          </div>
        </div>
      </div>

      <div className="flex-1 px-6 -mt-6 relative z-20 overflow-y-auto pb-24">
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-4 min-h-[300px]">
          <h3 className="font-semibold text-slate-800 mb-4 px-2">Today's Log</h3>
  ...
  const FoodsView = () => (
    <div className="flex flex-col h-full bg-slate-50 animate-in fade-in slide-in-from-bottom-4 duration-300">
      <div className="pt-10 pb-6 px-6 bg-white border-b border-slate-100 sticky top-0 z-10 flex justify-between items-center">
  ...
  const PhysicalPlatesView = () => (
    <div className="flex flex-col h-full bg-slate-50 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-3 pb-24">
        <p className="text-sm text-slate-500 mb-4">Register your physical plates/bowls to auto-deduct their empty weight when logging food.</p>
  ...
  // --- Modals ---
  const AddLogModal = () => {
    const [type, setType] = useState('food'); // 'food' or 'meal'
    
    // For 'food'
    const [selectedId, setSelectedId] = useState('');
    
    // For 'meal' (ad-hoc percentages)
    const [mealIngredients, setMealIngredients] = useState([{ foodId: '', percentage: '' }]);

    // For both (weight & plate deduction)
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex items-center justify-center sm:p-6 font-sans relative">
      ...
      <div className="w-full h-screen sm:h-[844px] max-w-[390px] bg-white relative sm:rounded-[40px] sm:shadow-2xl overflow-hidden flex flex-col sm:border-[8px] sm:border-slate-800">
        ...
        {/* Bottom Navigation */}
        <div className="h-20 bg-white border-t border-slate-100 flex items-center justify-around px-2 pb-safe z-40 relative">
          <button 
            onClick={() => setActiveTab('log')}
            className={`flex flex-col items-center justify-center w-1/3 h-full space-y-1 transition ${activeTab === 'log' ? 'text-emerald-600' : 'text-slate-400 hover:text-slate-600'}`}
          >
            <Home size={24} className={activeTab === 'log' ? 'fill-emerald-100' : ''} />
            <span className="text-[10px] font-medium tracking-wide">Log</span>
          </button>
          
          <button 
            onClick={() => setActiveTab('foods')}
            className={`flex flex-col items-center justify-center w-1/3 h-full space-y-1 transition ${activeTab === 'foods' ? 'text-emerald-600' : 'text-slate-400 hover:text-slate-600'}`}
          >
            <Utensils size={24} className={activeTab === 'foods' ? 'fill-emerald-100' : ''} />
            <span className="text-[10px] font-medium tracking-wide">Foods</span>
          </button>

          <button 
            onClick={() => setActiveTab('plates')}
            className={`flex flex-col items-center justify-center w-1/3 h-full space-y-1 transition ${activeTab === 'plates' ? 'text-emerald-600' : 'text-slate-400 hover:text-slate-600'}`}
          >
            <Disc size={24} className={activeTab === 'plates' ? 'fill-emerald-100' : ''} />
