...
if (isFirebaseConfigured) {
  ...
  // initializeFirestore + persistentLocalCache turns on the IndexedDB
  // offline cache. onSnapshot/setDoc/deleteDoc all keep working unchanged —
  // they just read/write through the cache and replay queued writes when
  ...
  try {
    ...
    // initializeFirestore can only run once per app instance. On Next.js
    // HMR / fast refresh it has already been called, so reuse the existing
    // instance via the imported getFirestore (no dynamic require, which
    // would crash in the client bundle).
...
const calcFoodCalories = (foods, foodId, grams) => {
  const food = foods.find(f => f.id === foodId);
  if (!food) return 0;
  return (grams / 100) * food.caloriesPer100g;
};

const getLogDetails = (log, foods) => {
  if (log.type === 'food') {
    const food = foods.find(f => f.id === log.itemId);
    const calories = calcFoodCalories(foods, log.itemId, log.amountGrams);
    return {
  ...
  const calories = (log.ingredients || []).reduce((totalCals, ing) => {
    ...
    return totalCals + calcFoodCalories(foods, ing.foodId, ingredientGrams);
  }, 0);
...
// =====================================================================
// MODALS — declared OUTSIDE App so their identity stays stable across
// parent re-renders. If they were inline in App, every Firestore snapshot
// (now firing twice with the offline cache: once from IndexedDB, once
// from the server) would unmount and remount them, wiping their input
// state on every keystroke. That was the "blank modal" bug.
// =====================================================================

function AddFoodModal({ onClose, user, setGlobalError }) {
...
function AddPhysicalPlateModal({ onClose, user, setGlobalError }) {
  const [name, setName] = useState('');
...
function AddLogModal({ onClose, user, foods, physicalPlates, currentDate, setGlobalError }) {
  ...
  if (netAmount > 0) {
    if (type === 'food' && selectedId) {
      previewCals = calcFoodCalories(foods, selectedId, netAmount);
      ...
      previewCals = mealIngredients.reduce((total, ing) => {
        ...
        return total + calcFoodCalories(foods, ing.foodId, ingredientGrams);
      }, 0);
    }
  }
  ...
  return (
    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-50 flex flex-col justify-end">
      <div className="bg-white rounded-t-[32px] p-6 pb-12 animate-in slide-in-from-bottom-8 duration-300 max-h-[90vh] overflow-y-auto">
        ...
        {/* Helpful empty-state hint when no foods exist yet */}
        {foods.length === 0 && (
          <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 mb-5 text-sm text-amber-800">
            You haven't registered any foods yet. Go to the <strong>Foods</strong> tab and tap <strong>Add</strong> first.
          </div>
        )}
...
export default function App() {
  ...
  const [trendsRange, setTrendsRange] = useState(7);
  ...
  // Online/offline detection (client-only to avoid hydration mismatch)
  useEffect(() => {
  ...
  const totalDailyCalories = dailyLogs.reduce(
    (total, log) => total + getLogDetails(log, foods).calories,
    0
  );
  ...
  const trendsData = useMemo(() => {
    ...
    for (let i = trendsRange - 1; i >= 0; i--) {
      ...
      const total = dayLogs.reduce(
        (sum, log) => sum + getLogDetails(log, foods).calories,
        0
      );
      result.push({
        date: dateStr,
  ...
  // --- Views (no local state, so safe to keep inline) ---
  const HomeView = () => (
      ...
      <div className="flex-1 px-6 -mt-5 relative z-20 overflow-y-auto pb-28">
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-4">
          ...
          {dailyLogs.length === 0 ? (
            ...
            <div className="space-y-3">
              {dailyLogs.map(log => {
                const details = getLogDetails(log, foods);
                return (
    ...
    <div className="h-[100dvh] sm:min-h-screen bg-slate-100 flex items-center justify-center sm:p-6 font-sans relative overflow-hidden">
      ...
      <div className="w-full h-full sm:h-[844px] max-w-[390px] bg-white relative sm:rounded-[40px] sm:shadow-2xl overflow-hidden flex flex-col sm:border-[8px] sm:border-slate-800">
        ...
        {showAddPhysicalPlate && (
          <AddPhysicalPlateModal
            onClose={() => setShowAddPhysicalPlate(false)}
            user={user}
            setGlobalError={setGlobalError}
          />
        )}
